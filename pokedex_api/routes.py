...
async def get_pokemon_route(start_index: int = Query(None, alias="start_index"),
    ...
    """
    pokemon_data = await get_pokemon_data(start_index=start_index, page_size=page_size, details=False)
    return pokemon_data
...
async def get_pokemon_data_route(start_index: int = Query(None, alias="start_index"),
                                 page_size: int = Query(None, alias="page_size")):
    """
    ...
    """
    pokemon_data = await get_pokemon_data(start_index=start_index, page_size=page_size, details=True)
    return pokemon_data
